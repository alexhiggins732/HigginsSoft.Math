...
namespace HigginsSoft.Math.Lib
{
    ...
    public class QsTest
    {
        ...
        {
            var a = MathUtil.GetNextPrime((GmpInt)(BigInteger)MathLib.GenerateRandomBigInt(16));
            var b = MathUtil.GetNextPrime((GmpInt)(BigInteger)MathLib.GenerateRandomBigInt(16));
    ...
    public class QuadraticSieve_Naive
    {
        private GmpInt n;
        ...
        public QuadraticSieve_Naive(GmpInt n)
        {
            this.n = n;
        }
        ...
        public GmpInt Factor()
        {
            ...
            for (var factorBaseBound = 50; factorBaseBound <= 5000; factorBaseBound += 25)
            {
                ...
                this.FactorBase = SelectFactorBase(factorBaseBound, n);
                int sieveInterval = 30000; // Example interval length.
                Console.Title = $"Factoring C{n.BitLength} with {FactorBase.Count} primes with {FactorBase.Count} primes and sieve size {sieveInterval}";
        ...
        private List<SmoothRelation> SieveForSmoothRelations(List<int> factorBase, GmpInt sqrtN, int sieveInterval)
        {
            ...
            for (int x = -sieveInterval; x <= sieveInterval; x++)
            {
                // Compute Q(x)
